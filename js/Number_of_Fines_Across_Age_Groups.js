...
// Function to create the age groups bar chart
function createAgeGroupsChart(data = window.dashboardData.ageGroups) {
    data = [...data];
    ...
    const width = document.getElementById('age-groups-chart').clientWidth - margin.left - margin.right;
    ...
    svg.selectAll('rect')
        ...
        .on("mouseover", function(event, d) {
            window.tooltip.transition()
                .duration(200)
                .style("opacity", .9);
            window.tooltip.html(`Age ${d.ageGroup}: ${d.fines.toLocaleString()} fines`)
...
window.createAgeGroupsChart = createAgeGroupsChart; 